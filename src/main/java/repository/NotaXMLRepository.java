...
public class NotaXMLRepository extends AbstractXMLRepository<Pair<String, String>, Nota> {
    ...
    public void createFile(Nota notaObj) {
        ...
        StudentFileRepository srepo = new StudentFileRepository(sval, "src/studenti.txt");
        TemaFileRepository trepo = new TemaFileRepository(tval, "src/teme.txt");

        Student student = srepo.findOne(idStudent);
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("src/" + student.getNume() + ".txt", false))) {
